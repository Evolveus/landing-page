...
const PILLARS = [
  {
    ...
    items: ['Bulk import + AI authoring', 'Per-student randomisation', 'Linear & adaptive quizzes'],
  },
  {
    n: 'II',
    title: 'Analytics',
    ...
    items: ['Live cohort dashboards', 'Per-question difficulty', 'Audit-ready exports'],
    flagship: true,
  },
  {
    n: 'III',
      ...
      <div className="p4-stage">
        ...
        <div className="p4-pillars">
          {PILLARS.map(({ n, title, sub, body, items, flagship }) => (
            <article key={n} className={`p4-pillar${flagship ? ' p4-pillar--flag' : ''}`}>
          ...
          <div className="p4-formats-list">
            {FORMATS.map((f, i) => (
              <span key={f} className="p4-format">
                <span className="p4-format-mono">{String(i + 1).padStart(2, '0')}</span>
                {f}
              </span>
            ))}
          </div>
